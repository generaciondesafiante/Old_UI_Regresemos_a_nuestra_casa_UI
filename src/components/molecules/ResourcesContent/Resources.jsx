import './Resources.css';

export const ResourcesComponent = () => {
  return (
    <>
      <div className="resources-container">
        <h1 className="resources-title">Recursos</h1>
        <div className="resources-content">
          <p className="resources-subtitle">Libros</p>
          <div className="resources-content_studyMaterial">
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
          </div>
        </div>
        <div className="resources-content">
          <p className="resources-subtitle">Documentales</p>
          <div className="resources-content_studyMaterial">
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
          </div>
        </div>
        <div className="resources-content">
          <p className="resources-subtitle">Estudios</p>
          <div className="resources-content_studyMaterial">
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
            <img
              className="resources-studyMaterial"
              src="https://static.wixstatic.com/media/d166cc_4cc837baf9254000a0f3963193c6b07a~mv2.jpg/v1/fill/w_368,h_195,al_c,q_80,usm_0.66_1.00_0.01,enc_auto/Romanos%2011111.jpg"
              alt=""
            />
          </div>
        </div>
      </div>
    </>
  );
};
